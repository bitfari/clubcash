...
;;  Give a 1,000,000 balance to buy digital land.

;;  Only 30 Tokens of this kind will ever be minted.
...
(define-constant FARI_CLUB_LIMIT u30)
...
;; Register a new token holder - 1M Club
(define-non-fungible-token club-1M uint)
...
;; SIP009: Transfer the token to a specified principal
 (define-public (transfer (token-id uint) (owner principal) (recipient principal))
  (if
    (and 
      (is-eq (some tx-sender) (nft-get-owner? club-1M token-id))
      (is-eq owner tx-sender)
    )
    (nft-transfer? club-1M token-id owner recipient)
    (err u500)
  )
)
...
;; SIP009: Get the owner of the specified token ID
 (define-read-only (get-owner (token-id uint))
   (ok (nft-get-owner? club-1M token-id)))
...
;; SIP009: Get the token URI
 (define-read-only (get-token-uri (token-id uint))
    (ok (some "https://bitfari.org/club-1M")))
    
;; Private functions
(define-private (is-owner (token-id uint) (user principal))
   (is-eq user (unwrap! (nft-get-owner? club-1M token-id) false))
)

;; Public functions
(define-public (burn (token-id uint))
   (if (is-owner token-id tx-sender)
     (match (nft-burn? club-1M token-id tx-sender)
       success (ok true)
       error (err error)
     )
       ERR_NOT_AUTHORIZED
...
;; Mint a New club-1M NFT
 (define-public (mint (new-owner principal)) 
    ...
    (match (nft-mint? club-1M next-id new-owner)
       success
       (begin
