...
;;  Give a 1K balance to buy digital land;
...
;; Register a new token holder - 1K Club
(define-non-fungible-token club-1k uint)
...
;; SIP009: Transfer the token to a specified principal
 (define-public (transfer (token-id uint) (owner principal) (recipient principal))
  (if
    (and 
      (is-eq (some tx-sender) (nft-get-owner? club-1k token-id))
      (is-eq owner tx-sender)
    )
    (nft-transfer? club-1k token-id owner recipient)
    (err u500)
  )
)
...
;; SIP009: Get the owner of the specified token ID
 (define-read-only (get-owner (token-id uint))
   (ok (nft-get-owner? club-1k token-id)))
...
;; SIP009: Get the token URI
 (define-read-only (get-token-uri (token-id uint))
    (ok (some "https://bitfari.org/club-1k")))
    
;; Private functions
(define-private (is-owner (token-id uint) (user principal))
   (is-eq user (unwrap! (nft-get-owner? club-1k token-id) false))
)

;; Public functions
(define-public (burn (token-id uint))
   (if (is-owner token-id tx-sender)
     (match (nft-burn? club-1k token-id tx-sender)
       success (ok true)
       error (err error)
     )
       ERR_NOT_AUTHORIZED
...
;; Mint a New club-1k NFT
 (define-public (mint (new-owner principal)) 
    ...
    (match (nft-mint? club-1k next-id new-owner)
       success
       (begin
